        ...
        for i = 1:length(subDirs)
            ...
            try
                ...
                if ~isfile(ctx1) || ~isfile(ctx2)
                    % PART2 clip failed for this pair (e.g. CRS error) -- no
                    % valid tifs were ever produced. Not a real result, so no
                    % pairsinfo row; just clean up the empty/incomplete folder.
                    fprintf('  Missing tif files for %s, deleting folder.\n', name_root);
                    cd(folderPath);
                    rmdir(pairDir, 's');
                    continue
                end
                ...
                if n_hits < 31
                    ...
                    fprintf('    Skipping JPEG save (%d hits exceeds threshold), deleting folder.\n', n_hits);
                end
